...
export default class Search {
  // 1. Select DOM elements, and keep track the data
  constructor() {
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      <div class="list-group-item active"><strong>Search Results</strong> (${posts.length > 1 ? `${posts.length + 1} items found` : '1 item found'})</div>
      ...
      this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm"> Sorry, we could not find any results for that keyword. </p>`
    }
    this.hideLoaderIcon();
