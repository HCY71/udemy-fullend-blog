...
export default class Registration {
    ...
    events() {
        ...
        this.username.addEventListener('keyup', () => {
            this.isValueChange(this.username, this.usernameHandler);
        })
        this.email.addEventListener('keyup', () => {
            this.isValueChange(this.email, () => this.emailHandler());
        })
        this.password.addEventListener('keyup', () => {
            this.isValueChange(this.password, () => this.passwordHandler());
        })
    ...
    passwordImmediately() {
        if (this.password.value.length > 50) {
            this.showValidationError(this.password, "Password should not exceed 50 characters.");
        }
    ...
    usernameAfterDelay() {
        ...
        if (!this.username.errors) {
            axios.post('/doesUsernameExist', {_csrf: this._csrf, username: this.username.value}).then((response) => {
                if (response.data) {
                    this.showValidationError(this.username, "The username has been already taken.");
                    this.username.isUnique = false;
                } else {
        ...
        if (!this.email.errors) {
            axios.post('/doesEmailExist', {_csrf: this._csrf, email: this.email.value})
                .then((response) => {
                    if (response.data) {
                        this.email.isUnique = false;
                        this.showValidationError(this.email, "The email address has already been uses.");
                    } else {
